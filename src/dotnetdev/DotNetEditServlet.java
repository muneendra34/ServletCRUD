 ...
 */
@WebServlet("/DotNetEditServlet")
public class DotNetEditServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try 
		{	
			int i = dna.goDotNetUpdate(fn,ln,psw,rpsw,addr,phone,des,ofn); 
			if( i != 0)
			{
				out.print("Successfully record updated!!");  
				RequestDispatcher rd = request.getRequestDispatcher("DotNetIndex1");
	            rd.forward(request, response); 
			}
			else
			{
				response.sendRedirect("dotnet/dotnetupdateerror.html");
			}
